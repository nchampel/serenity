...
try {
    ...
    $quantityWeaponDB = $getEnemyInfos['quantityWeaponDB'];
    $quantityLifePointsDB = $getEnemyInfos['quantityLifePointsDB'];
    $crystal = $getEnemyInfos['crystal'];

    if ($quantityWeaponDB['status'] && $quantityLifePointsDB['status']) {
        $Response['status'] = 200;
        $Response['data'] = ['data' => ['life_points' => $quantityLifePointsEnemy, 'power' => $quantityWeaponEnemy, 'crystal' => $crystal]];
